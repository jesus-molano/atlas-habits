...
function GlobalTimerSurface() {
  ...
  const run = async (operation: () => Promise<AdapterActionResult>) => {
        ...
        {legacyItems.length > 1 && !timer ? (
          <View style={styles.timerSection}>
            {legacyItems.map((habit) => (
              <Button
                ...
                onPress={() => void run(() => resolveLegacyTimers(habit.id))}
                variant="secondary"
              />
            ))}
            <Button
              ...
              leadingIcon={Trash2}
              onPress={() => void run(() => resolveLegacyTimers(null))}
              variant="danger"
            />
          </View>
        ) : timer ? (
          <View style={styles.timerSection}>
            ...
            <View style={styles.timerActions}>
              <Button
                ...
                onPress={() =>
                  void run(timer.runningSince ? pauseTimer : resumeTimer)
                }
                variant="secondary"
              />
              <Button
                ...
                leadingIcon={Square}
                onPress={() =>
                  void run(() => stopTimer(dateKey(new Date()))).then(
                    (result) => result.ok && close(),
                  )
                }
              />
              <Button
          ...
          <View style={styles.timerSection}>
            ...
            <Button
              ...
              leadingIcon={Play}
              onPress={() =>
                selected && void run(() => startTimer(selected.id))
              }
            />
